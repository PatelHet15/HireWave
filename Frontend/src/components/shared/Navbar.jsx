...
import { LogOut, Settings, HelpCircle, Sparkles, Clock, Dot, ArrowRight, ChevronDown, ChevronUp, Bell, BellIcon, MapPin, ListFilter, Briefcase, Building2, Users, BarChart, LayoutDashboard, Search, X, User, Bookmark, Menu } from "lucide-react";
...
const Navbar = () => {
  ...
  const handlePopoverOpenChange = (open) => {
    ...
    if (!open && user?.notifications?.some(n => !n.isRead)) {
      ...
      axios.post(`${USER_API_END_POINT}/markNotificationsAsRead`, {}, { 
        withCredentials: true 
      })
      .then(() => {
  ...
  const logOutHandler = async () => {
    try {
      ...
      // Whether the API call succeeds or not, clean up local state
      dispatch(setUser(null));
      localStorage.removeItem('user');
      localStorage.removeItem('token');
        ...
        <ul className="hidden md:flex items-center gap-8 font-medium text-base">
          {user?.role === "recruiter" && (
            <>
              ...
              <li>
                <Link to="/admin/candidates" className="hover:text-blue-600 transition-colors flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-blue-50">
                ...
                <div className="space-y-4">
                  {user?.role === "recruiter" && (
                    <>
                      ...
                      <Link to="/admin/candidates" className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg">
                                ...
                                <div className="flex items-start gap-2">
                                  {notification.companyLogo ? (
                                    <Avatar className="h-8 w-8 rounded-md ring-1 ring-gray-100">
                                      <AvatarImage src={notification.companyLogo} alt="Company" />
                                    </Avatar>
                                  ) : (
                ...
                <div className="relative cursor-pointer">
                  {user.role === "student" ? (
                    <div className="relative w-10 h-10">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={user?.profile?.profilePhoto || logo} alt="User Avatar" />
                        </Avatar>
                      </div>
                    </div>
                  ) : (
                    ...
                    <Avatar className="h-10 w-10 ring-2 ring-gray-100">
                      <AvatarImage 
                        src={user?.profile?.company?.logo || logo} 
                        alt="Company Logo"
                        onError={(e) => {
