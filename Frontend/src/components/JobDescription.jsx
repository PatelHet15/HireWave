...
const JobDescription = () => {
  ...
  const applyJobHandler = async () => {
    try {
      ...
      try {
        if (user?._id && (user.role === 'candidate' || user.role === 'student')) {
          console.log('Tracking apply click for job ID:', jobId, 'User role:', user.role);
          await axios.post(`${ANALYTICS_API_END_POINT}/job/${jobId}/apply-click`, {}, {
          ...
          console.log('Apply click tracked successfully');
        } else {
          console.log('Apply click not tracked: User not logged in or not a candidate/student');
        }
      } catch (analyticsError) {
        console.error("Error tracking apply button click:", analyticsError);
  ...
  const trackJobView = async () => {
    ...
    // Only track views for candidates and students
    if (user.role !== 'candidate' && user.role !== 'student') {
      console.log('Job view not tracked: User role is not candidate or student');
      return;
    }
    
    try {
      console.log('Tracking job view for job ID:', jobId, 'User role:', user.role);
  ...
  const fetchSingleJob = async () => {
    try {
      ...
      if (response.data.success) {
        // The job data is in response.data.job, not response.data.data
        dispatch(setSingleJob(response.data.job));
        
        // Similar jobs are already included in the response
        if (response.data.similarJobs) {
          dispatch(setSimilarJobs(response.data.similarJobs));
        }
        
        // Check if job is already saved in bookmarks
        const savedBookmarks = localStorage.getItem('bookmarkedJobs');
        if (savedBookmarks) {
          const bookmarks = JSON.parse(savedBookmarks);
          setIsSaved(!!bookmarks[jobId]);
        }
        ...
        if (user?._id) {
          ...
          if (applicationResponse.data.success) {
            // The hasApplied property is directly in the response, not under data
          ...
          <div className="lg:w-[70%] w-full space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex justify-between">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    ...
                    >
                      {isSaved ? <BookmarkCheck size={20} className="text-blue-600" /> : <Bookmark size={20} />}
                    </button>
                    <button 
                      className="p-2 rounded-full text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition-colors"
              ...
              {daysLeft !== null && daysLeft > 0 && (
                <div className={`mt-5 py-2 px-3 rounded-lg text-sm ${
                  daysLeft <= 3 
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">Apply now and take the next step in your career journey.</p>
                </div>
                <Button
                  onClick={applyJobHandler}
                  disabled={isApplied || isApplying}
