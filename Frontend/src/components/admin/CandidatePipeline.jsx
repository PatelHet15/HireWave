                  ...
                  <div className="rounded-xl p-5 border border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-sm transition-transform transform group-hover:-translate-y-1 group-hover:shadow-md duration-300">
                    ...
                    <div className="flex items-center justify-between text-sm font-medium text-gray-700">
                      <span className="text-green-600">
                        {candidates.filter(c => c.currentRound === round._id).length} Candidates
                      </span>
                      <span className="text-blue-500">
                        {
                          candidates.filter(c => {
                            if (!c.roundsStatus || !Array.isArray(c.roundsStatus)) return false;
                            const roundStatus = c.roundsStatus.find(
                              rs => rs && rs.round && String(rs.round) === String(round._id)
                            );
                            return roundStatus && roundStatus.status === 'passed';
                          }).length
                        } Passed
