...
export const updateApplicationStatus = async (req, res) => {
  try {
    ...
    try {
      ...
      // PATCH: When accepted, set currentRound and roundsStatus if not already set
      if (status === 'accepted') {
        const job = application.job;
        if (job && job.interviewRounds && job.interviewRounds.length > 0) {
          const firstRound = job.interviewRounds[0];
          if (!application.currentRound) {
            application.currentRound = firstRound._id;
          }
