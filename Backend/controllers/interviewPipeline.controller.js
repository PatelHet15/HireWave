...
export const addInterviewRounds = async (req, res) => {
  try {
    ...
    // Accept both request formats: {rounds: []} or {interviewRounds: []}
    const rounds = req.body.rounds || req.body.interviewRounds || [];
    const userId = req.id;
    ...
    if (!Array.isArray(rounds)) {
      console.error('Invalid rounds data format (not array):', typeof rounds);
      return res.status(400).json({
        message: "Interview rounds must be provided as an array",
        success: false,
        receivedType: typeof rounds
      });
    }
    ...
    const interviewRounds = await Promise.all(rounds.map(async (round, index) => {
      const newRound = {
        ...round,
    ...
    return res.status(200).json({
      ...
      roundsCount: interviewRounds.length
    });

  } catch (error) {
    console.error("Detailed error adding interview rounds:", {
      error: {
        name: error.name,
    ...
    return res.status(500).json({
      message: "Internal server error while adding interview rounds",
      success: false,
...
export const reorderInterviewRounds = async (req, res) => {
  try {
    const { jobId } = req.params;
    const { roundIds } = req.body; // Array of round IDs in the new order
    const userId = req.id;
...
export const addCandidateToProcess = async (req, res) => {
  try {
    ...
    const job = await Job.findById(jobId).populate('interviewRounds');
